...
private:
   ...
   // Starting and ending track times
   ...
   PlaybackSchedule &schedule, double offset )
{
   ...
   if (offset >= 0) {
      auto space = std::max(0.0, std::min(offset, mGapLeft - time));
      time += space;
      offset -= space;
      if (offset > 0)
      ...
      auto space =
         std::min(0.0, std::max(offset, mGapLeft + mGapLength - time));
      time += space;
      offset -= space;
      if (offset < 0)
         time = std::min(time, mGapLeft) + offset;
   }
   time = std::min( std::max(mStart, mEnd),
      std::max( std::min(mStart, mEnd), time ) );
